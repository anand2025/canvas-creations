...
const fetchSellerOrders = (params) => api.get('/seller/orders', { params });
const updateSellerOrderStatus = (id, status) => api.put(`/seller/orders/${id}/status?status=${status}`);
...
const SellerOrdersPage = () => {
  ...
  const {
    ...
    pagination,
    updateOrderStatus
  } = useOrders(fetchSellerOrders);

  const handleStatusChange = (orderId, newStatus) => {
    updateOrderStatus(orderId, newStatus, updateSellerOrderStatus);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-foreground/50">Manage your sales and track your masterpieces</p>
        </div>
      </div>

      {/* Filters (Standardized with Admin) */}
