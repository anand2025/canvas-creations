...
const SellerLayout = ({ children }) => {
  ...
  if (isLoading) {
    return <div className="flex h-screen items-center justify-center bg-gray-100 dark:bg-gray-900 text-gray-500">Loading Seller Panel...</div>;
  }
  ...
  const navItems = [
    ...
    { name: 'My Orders', href: '/seller/orders', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
    )},
    { name: 'Product Reviews', href: '/seller/reviews', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.54 1.118l-3.976-2.888a1 1 0 00-1.175 0l-3.976 2.888c-.784.57-1.838-.197-1.539-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.382-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path></svg>
    )},
  ];
    ...
    <div className="flex h-screen bg-background font-sans text-foreground transition-colors duration-500">
      ...
      >
        <div className="h-16 flex items-center justify-between px-4 border-b border-[var(--border-color)]">
          {isSidebarOpen ? (
            <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-vibrant-teal to-vibrant-blue truncate">
              SELLER PANEL
            </span>
          ) : (
            <span className="text-xl font-bold text-vibrant-teal mx-auto">SP</span>
          )}
          <button 
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <header className="h-16 bg-card shadow-sm border-b border-[var(--border-color)] flex items-center justify-between px-4 md:hidden">
           <span className="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-vibrant-teal to-vibrant-blue">
              CANVAS SELLER
           </span>
