          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-6">
              ...
              <div className="bg-secondary-bg/50 p-6 rounded-3xl border border-[var(--border-color)] space-y-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-foreground/40 block mb-1">Name</label>
                  <p className="font-bold text-lg">{order.customer_name || 'Guest'}</p>
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-foreground/40 block mb-1">Email</label>
                  <p className="font-bold">{order.customer_email || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-secondary-bg/50 rounded-3xl border border-[var(--border-color)] overflow-hidden">
              <table className="w-full text-left">
                <thead className="bg-secondary-bg/80 border-b border-[var(--border-color)]">
                  <tr>
                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-foreground/40">Item ID</th>
                ...
                <tbody className="divide-y divide-[var(--border-color)]">
                  {order.items.map((item, index) => (
                    <tr key={index} className="hover:bg-secondary-bg/80 transition-colors">
                      <td className="px-6 py-4 font-mono text-xs text-foreground/60">{item.painting_id}</td>
                ...
                <tfoot className="bg-secondary-bg/30 font-black">
                  ...
                  <tr>
                    ...
                    <td className="px-6 py-3 text-right text-vibrant-teal">+ ₹{(order.shipping_cost || 0).toLocaleString()}</td>
                  </tr>
                  <tr className="border-t-2 border-[var(--border-color)]">
                    ...
                    <td className="px-6 py-6 text-right text-2xl text-vibrant-teal">₹{order.grand_total.toLocaleString()}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
